...
P1Accessory.prototype.check = function (data) {
  ...
  if (this.type === 'E') {
    if (data.consumption.normal !== this.state.consumptionNormal) {
      ...
      this.service.updateCharacteristic(
        my.Characteristic.TotalConsumptionNormal, this.state.consumptionNormal
      )
    }
    if (data.consumption.low !== this.state.consumptionLow) {
      ...
      this.service.updateCharacteristic(
        my.Characteristic.TotalConsumptionLow, this.state.consumptionLow
      )
    }
  }
  ...
  if (
    ...
    data.l1.current !== this.state.current
  ) {
  ...
  if (
    ...
    data.l1.voltage !== this.state.voltage
  ) {
