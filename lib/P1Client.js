...
class P1Client extends EventEmitter {
  ...
  async open () {
    ...
    if (hostname != null && port != null) {
      ...
      this.emit(
        'warning',
        `${serialPort}: connecting to a serial port device might be removed in Homebridge P1 v2, see https://github.com/ebaauw/homebridge-p1/issues/83`
      )
    ...
    this.p1
      .on('data', (buffer) => {
        ...
        let position
        while ((position = this.s.indexOf('/')) !== -1) {
          this.parseTelegram(this.s.slice(0, position).toString())
          this.s = this.s.slice(position + 1)
        }
      })
      .on('close', () => {
