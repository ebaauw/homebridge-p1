...
const { OptionParser, toHexString } = require('homebridge-lib')
...
function parseAvgPowerPeak(values) {
  const peaks = {}
  peaks[parseTimestamp(values[0])] = parseValue(values[1])
  return peaks
}
...
function parseAvgPowerPeaks (values) {
  ...
  const entries = parseValue(values[0])
  for (let i = 1; i <= entries; i++) {
    peaks[parseTimestamp(values[3 * i + 1])] = parseValue(values[3 * i + 2])
  }
  return peaks
}
  ...
  parseTelegram (telegram) {
    try {
      ...
      if (obj.avg_power_peak != null) {
        ...
        const key = Object.keys(obj.avg_power_peak)[0]
        obj.avg_power_peaks[key] = obj.avg_power_peak[key]
      }
      const result = {
        type: obj.type,
