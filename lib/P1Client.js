...
module.exports = class P1Client extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      SerialPort.list().then((ports) => {
        ...
        for (const port of ports) {
          if (
            (this.host == port.comName) ||
            (this.host == "" && port.vendorId === '0403' && port.productId === '6001') ||
            (this.host == "" && port.vendorId === '067b' && port.productId === '2303') // Issue #7
          ) {
            const p1 = new SerialPort(
              port.comName, {baudRate: 115200}, (error) => {
                if (error) {
  ...
  parseData (data) {
    ...
    const type = data.match(/(.+)\r?\n\r?\n/)
    const checksum = data.match(/!(....)\r?\n/)
    if (type == null || checksum == null) {
      return null
    }
    // TODO: validate checksum
    this.emit('telegram', data)
    const obj = {type: type[1]}
