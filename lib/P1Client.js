...
const p1Keys = {
  ...
  '0-0:96.1.4': [{ key: 'id_be', f: parseValue }],
  '0-0:96.3.10': [{ key: 'switch', f: parseValue }],
  ...
  '0-1:24.4.0': [{ key: 'd1_switch', f: parseValue }],
  ...
  '0-2:24.4.0': [{ key: 'd2_switch', f: parseValue }],
  ...
  '0-3:24.4.0': [{ key: 'd3_switch', f: parseValue }],
  ...
  '0-4:24.4.0': [{ key: 'd4_switch', f: parseValue }],
...
class P1Client extends EventEmitter {
  ...
  /** Open the connection to the serial port.
    ...
    */
  async open () {
    const port = this._options.serialPort != null
    ...
    return new Promise((resolve, reject) => {
      this.p1 = new SerialPort(port, options, (error) => {
        if (error) {
  ...
  parseTelegram (telegram) {
    try {
      ...
      const header = telegram.match(/(.+)\r?\n\r?\n/)
      const footer = telegram.match(/(!)(.*)\r?\n/)
      ...
      const be = obj.id_be != null
      const low = be ? 2 : 1
      const normal = be ? 1 : 2
      const tariff = obj.tariff === low ? 'low' : 'normal'
      const result = {
        type: obj.type,
        version: be ? 'B' : obj.version == null ? '2.2' : obj.version,
