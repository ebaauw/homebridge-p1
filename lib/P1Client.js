...
function parseAvgPowerPeak (values) {
  return {
...
function parseAvgPowerPeaks (values) {
  const peaks = []
  const entries = parseValue(values[0])
  for (let i = 1; i <= entries; i++) {
    peaks.push({
      power: parseValue(values[3 * i + 2]),
      time: parseTimestamp(values[3 * i + 1])
    })
  }
  return peaks
}
...
const p1Keys = {
  ...
  '0-0:98.1.0': [{ key: 'avg_power_peaks', f: parseAvgPowerPeaks, allValues: true }],
  ...
  '1-0:1.6.0': [{ key: 'avg_power_peak', f: parseAvgPowerPeak, allValues: true }],
...
class P1Client extends EventEmitter {
  ...
  #toHost (serialPort) {
    try {
      return OptionParser.toHost('serialPort', serialPort)
    } catch (error) { return {} }
  }
  ...
  async open () {
    this.s = ''
    let serialPort = this._options.serialPort
    const { hostname, port } = this.#toHost(serialPort)
    if (hostname != null && port != null) {
      this.#setTimeout()
      this.p1 = createConnection(port, hostname)
      this.p1.close = this.p1.destroy
      await once(this.p1, 'ready')
      this.log('connected to %s', serialPort)
    } else {
      this.warn(
        `${serialPort}: loading 'serialport' library, see https://github.com/ebaauw/homebridge-p1/issues/84`
      )
      const { SerialPort } = await import('serialport')
      if (serialPort == null) {
        const ports = await SerialPort.list()
        serialPort = this.#findPort(ports)
      }
      ...
      this.#setTimeout()
      this.p1 = new SerialPort(options)
  ...
  #setTimeout () {
    ...
    this.timeout = setTimeout(() => {
      if (this.p1 != null) {
        this.warn(`no data received in ${this._options.timeout}s`)
        this.p1.close()
        delete this.p1
      }
  ...
  parseTelegram (telegram) {
    this.vvdebug('telegram:\n%s', telegram)
    try {
      // Validate telegram.
      if (telegram == null || typeof telegram !== 'string') {
        this.warn('ignoring unsupported telegram')
        return
      }
      ...
      if (footer[2] != null) {
        ...
        if (checksum !== crc) {
          const got = toHexString(checksum, 4)
          const expected = toHexString(crc, 4)
          this.emit(
            'warning',
            `ignoring telegram with crc error (got: ${got}, expected: ${expected})`
          )
          return
        }
      }
      ...
      for (const line of lines) {
        ...
        if (a == null) {
          /** Emitted when the telegram contains an unknown key.
            ...
            */
          this.warn(`${keys[0]}: ignoring unknown key`)
          continue
        }
        for (const id in a) {
          if (a[id].key != null) {
            try {
              ...
              this.warn(`${keys[0]}: ignoring unknown value ${value}`)
            }
          }
        }
      }
      this.vvdebug('raw data: %j', obj)
      ...
      const result = {
        ...
        electricity: {
          ...
          l1: {
            ...
            current: (obj.l1_power_back > 0) ? 0 : obj.l1_current,
            power: obj.l1_power
          }
        },
        electricityBack: {
          ...
          tariff,
          consumption: {
            low: obj['consumption_low_t' + low],
            normal: obj['consumption_back_t' + normal]
          },
          power: obj.power_back,
          l1: {
            voltage: obj.l1_voltage,
            current: (obj.l1_power_back > 0) ? obj.l1_current : 0,
      ...
      if (obj.l2_power != null && obj.l3_power != null) {
        result.electricity.l2 = {
          ...
          current: (obj.l2_power_back > 0) ? 0 : obj.l2_current,
          power: obj.l2_power
        }
        result.electricityBack.l2 = {
          voltage: obj.l2_voltage,
          current: (obj.l2_power_back > 0) ? obj.l2_current : 0,
        ...
        result.electricity.l3 = {
          ...
          current: (obj.l3_power_back > 0) ? 0 : obj.l3_current,
          power: obj.l3_power
        }
        result.electricityBack.l3 = {
          voltage: obj.l3_voltage,
          current: (obj.l3_power_back > 0) ? obj.l3_current : 0,
