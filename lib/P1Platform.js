  ...
  constructor (log, configJson, homebridge) {
    ...
    try {
      ...
      if (this.config.telegram != null) {
        ...
        this.connectWs = -1
        this.ws.on('error', (error) => { this.warn(error) })
        this.ws.on('connect', (url) => { this.log('connected to %s', url) })
        this.ws.on('disconnect', (url) => {
          this.connectWs = this.beat + 60
          this.log('disconnected from %s', url)
        })
        this.ws.on('data', (data) => { this.onData(data) })
        if (this.config.wsPort != null) {
        ...
        this.p1.once('telegram', (s) => { this.debug('telegram:\n%s', s) })
        this.p1.once('rawData', (obj) => { this.debug('raw data: %j', obj) })
        ...
        if (this.config.wsPort != null) {
          ...
          this.server.on('server: listening', (port) => {
            this.log('listening on port %d', port)
          })
          this.server.on('close', (port) => {
