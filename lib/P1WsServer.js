...
class P1WsServer extends EventEmitter {
  /** Create a new instance of a P1 web socket server.
    ...
    * @param {int} [port=8088] - The port for the web socket server.
    */
  constructor (p1, options = {}) {
    ...
    p1.on('telegram', (telegram) => { this.onEvent('telegram', telegram) })
    p1.on('rawData', (data) => { this.onEvent('rawData', data) })
    p1.on('data', (data) => { this.onEvent('data', data) })
